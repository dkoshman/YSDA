...
public:
    ...
    explicit Output(std::vector<int32_t> z_function) : z_function{std::move(z_function)} {
    }
...
std::vector<int32_t> PrefixToZFunction(const std::vector<int32_t> &prefix_function) {

    auto size = static_cast<int32_t>(prefix_function.size());
    std::vector<int32_t> z_function(size);
    z_function[0] = size;

    for (int32_t i = 1; i < size; ++i) {
        if (prefix_function[i] > 0) {
            int32_t z_peak_index = i - prefix_function[i] + 1;
            z_function[z_peak_index] = std::max(z_function[z_peak_index], prefix_function[i]);
        }
    }

    for (int32_t index = 0, shadow_of_z_peak_begin = 0; index < size; ++index) {
        if (z_function[index] != 0) {
            shadow_of_z_peak_begin = index;
            continue;
        }

        auto shadow_end = shadow_of_z_peak_begin + z_function[shadow_of_z_peak_begin];
        if (index > shadow_end) {
            continue;
        }

        int32_t sprout_z_peak_shadow = z_function[index - shadow_of_z_peak_begin];
        if (index + sprout_z_peak_shadow <= shadow_end) {
            z_function[index] = sprout_z_peak_shadow;
        } else {
            z_function[index] = shadow_end - index;
        }
    }
...
io::Output Solve(const io::Input &input) {
    return io::Output{PrefixToZFunction(input.prefix_function)};
}
...
TestIo GenerateRandomTestIo(int32_t test_case_id) {
    ...
    io::Input input;
    input.prefix_function = CalculatePrefixFunctionForString(string);
    auto z_function = CalculateZFunctionForString(string);

    return TestIo{input, io::Output{z_function}};
}
